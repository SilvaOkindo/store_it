    ...
    <>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="auth-form">
          ...
          {errorMessage && (
            <p className="shad-form-message">*{errorMessage}</p>
          )}
          <div className="body-2 flex justify-center">
            <p className="text-light-100">{type === "sign-in" ? "Don't have an account" : "Already have an account?"}</p>
            <Link href={type === "sign-up" ? "/signin" : "/signup"} className="ml-2 font-medium text-brand">{type==="sign-up" ? "sign in" : "sign up"}</Link>
          </div>
        </form>
        {/* OTP */}
